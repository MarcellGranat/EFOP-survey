...
knitr::opts_chunk$set(echo = TRUE, message = F, warning = F, comment = "", fig.align = "center", fig.width = 9, fig.height = 6, cache = T, fig.path='plot/', tidy = T, cache.path="cache/")
```
...
```{r}
skimr::skim(setNames(survey, c(str_c(df_names$variable, ": ", df_names$english))))
```
...
```{r fig.height= 12}
survey %>% select(employed:time_employed) %>% gather() %>% merge(df_names, by.x = "key", by.y = "variable") %>% filter(!is.na(value)) %>% 
...
cramer_matrix <- cramer_matrix %>% data.frame() %>% 
  rownames_to_column() %>% pivot_longer(-1) %>% transmute(
